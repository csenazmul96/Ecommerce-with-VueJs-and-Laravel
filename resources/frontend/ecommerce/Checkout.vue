                    ...
                    <div class="col-12">
                        <div class="checkout_step_head_wrap">
                            <div class="checkout_step_head">
                                <ul v-if="isLoggedIn && shippingInput.id != ''">
                                ...
                                <ul v-if="(!isLoggedIn) || (isLoggedIn && shippingInput.id === '')">
                        ...
                        <div class="hl_checkout_left">
                            ...
                            <div class="step_2" :class="{'d-block':activeTab === 'shipping'}">
                                ...
                                <div class="c_custom_section">
                                    <div class="inner" v-for="(shipMethod, shipMethodIndex) in shippingMethods" :key="'shipMethod_' + shipMethodIndex">
                                        ...
                                        <span v-if="shipping_free && shipMethod.type === 'Standard'"><span class="text-danger">(Free Shipping)</span></span>
                                        <span v-else>{{shipMethod.fee ? ('USD$ ' + shipMethod.fee) : 'Actual Cost'}}</span>
                                    </div>
                            ...
                            <div class="step_3" :class="{'d-block':activeTab === 'payment'}">
                                ...
                                <div class="inner mb_10">
                                    <paypal :shippingMethod="shippingMethod" :billingInfo="billingInput" :cartTotal="computedSubTotal" :shippingCost="computedShipping" :totalAmount="computedTotal" :shippingInput="shippingInput" :cartItems="cartItems" :user="userInput"  :countries="countries" :states="states" :discount="computedDiscount | round(2)"></paypal>
                                </div>
                                ...
                                <div class="d_flex_end c_shipping m_none">
                                    <router-link :to="{ name: 'cart'}">Return to cart</router-link>
                                    <button class="btn_common width_200p step_3_btn checkout_btn" :class="{disable:buttonDisable}" @click="checkout($event,2, null)"> Continue to Checkout <i v-if="buttonDisable" class="fas fa-spinner fa-pulse"></i> </button>
                                </div>
...
<!--                                    <div class="float_right width_200p step_3_btn paypal-button-container" :disabled="checkoutLoader" v-if="cartItems && cartItems.length > 0"></div>-->
                    ...
                    <div class="col-md-5 col-lg-4">
                        <div class="hl_checkout_right above_mobile">
                            ...
                            <div class="hl_total_show" v-if="cartItems && cartItems.length > 0">
                                <table>
                                    ...
                                    <tr>
                                        <td>Shipping
                                            <small class="text-danger" v-if="shipping_free"> (Free Shipping)</small>
                                        </td>
                        ...
                        <div class="d_flex_end cs_btn_mobile ipad_none pb_20">
                            ...
                            <button v-if="activeTab === 'payment'" @click="checkout($event,2, null)" class="btn_common checkout_btn" :class="{disable:buttonDisable}"> Continue to Checkout <i v-if="buttonDisable" class="fas fa-spinner fa-pulse"></i></button>
                        </div>
                    </div>
                </div>
                ...
                <div class="row">
                    ...
                    <div class="col-12">
                        <div class="hl_footer_copyright">
                            <p>© Copyright 2021 Hologram Nails Inc. All Rights Reserved | Email : info@shophologram.com</p>
                            <p>Register Office : 3761 S Hill St Unit 1, Los Angeles, CA 90007, USA </p>
                        </div>
                    </div>
                </div>
            </div>
...
import paypal from "./components/PaypalForm";
export default {
    name:'checkout',
    mixins: [mixin],
    components:{
    ...
    computed:{
        ...
        computedShipping () {
            if(this.shippingMethod && (this.shippingMethod.type === 'Standard') && this.promoCode && this.promoCode.free_shipping == 1) {
                this.shipping_free = true;
                return 0.00;
            }else if(this.shippingMethod && (this.shippingMethod.type === 'Expedited') && this.promoCode && this.promoCode.free_shipping == 1) {
                this.shipping_free = false;
                return this.shippingMethod.fee;
            }else if(this.shippingMethod && this.shippingMethod.fee) {
                return this.shippingMethod.fee;
            }
            return 0.00;
        },
    ...
    methods:{
        ...
        checkout(event, paymentMethod, paymentId){
            ...
            let formData = {
                ...
                freeShipping: this.shipping_free,
                billingShippnigSame: this.shippingToBilling,
                paymentId: paymentId
            }
            this.$store.dispatch('ecommerceStore/checkout', formData).then((res)=>{
                this.checkoutLoader = null
                this.$Progress.finish()
                this.buttonDisable = false;
            }).catch((error) => {
                this.buttonDisable = false;
            }).finally(()=>{this.buttonDisable = false;});
