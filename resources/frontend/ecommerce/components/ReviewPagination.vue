<template>
    <div>
        <ul>
            <li>
                <div v-if="paginateData.current_page == 1">
                    <a href="javascript:void(0)"><i class="fas fa-angle-left"></i></a>
                </div>

                <div v-else>
                    <a href="javascript:void(0)" @click="changePage(Number(paginateData.current_page) - 1)"><i class="fas fa-angle-left"></i></a>
                </div>
            </li>
            <li v-for="page in pagesNumber" :key="'pagination-' + page" :class="{'active' : (page == paginateData.current_page) || (pagesNumber.length == 1)}">
                <a href="javascript:void(0)" @click="changePage(page)">{{ page }}</a>
            </li>
            <li>
                <div v-if="paginateData.current_page == paginateData.last_page">
                    <a href="javascript:void(0)"><i class="fas fa-angle-right"></i></a>
                </div>
                <div v-else>
                    <a href="javascript:void(0)" @click="changePage(Number(paginateData.current_page) + 1)"><i class="fas fa-angle-right"></i></a>
                </div>
            </li>
        </ul>
    </div>
