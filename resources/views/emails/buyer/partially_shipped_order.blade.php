...
<head>
    ...
    <title>Partially Shipped Order</title>
</head>
    <style>
        ...
        <table style="table-layout: fixed;">
            <tr>
                <td style="text-align: center;">
                    ...
                    @endif
                    <h1 style="margin-top: 30px; margin-bottom: 40px; font-size: 30px; text-transform: uppercase; font-weight: 500;">SHIPPING DETAILS</h1>
                    <h4 style="margin-top: 0px; margin-bottom: 40px; font-size: 15px;">Your order from Hologram Nails has been shipped.</h4>
                    <p style="text-align: left; font-weight: 400;"> <span style="font-weight: 300;">Dear</span>  ({{ $order->user->first_name }} {{ $order->user->last_name }}) <br> your order has been partially shipped. <br>
                        <span>Shipping Method : </span>
                        @if(!empty($order->shippingMethod->courier))
                            {{ $order->shippingMethod->courier->name }} |
                        @endif
                        @if(!empty($order->shippingMethod->name))
                            <span> {{ $order->shippingMethod->name }}</span>
                        @endif
                        <br>
                        @if(!empty($order->tracking_number))
                            <span>Tracking Number : </span> <span>{{ $order->tracking_number }}</span>
                        @endif
                    </p>
                </td>
            </tr>
                ...
                <td style="width: 100%;">
                    <table style="font-weight: 300; width: 100%; text-align: left;">
                        <tr>
                            ...
                            <th style="border-bottom:1px solid #EFEFEE; padding-bottom: 10px; font-weight: 300; width: 30%;">Name</th>
            ...
            <tr>
                <td>
                    <p style="font-weight: 300; border-top: 2px solid #000; padding-top: 20px;">Need Help? Please contact our Customer Service Team at: info@shophologram.com</p>
                    <p style="font-weight: 300; margin-bottom: 30px;"><span style="font-weight: 400;">Best,</span>  <br> Shop Hologram </p>
                </td>
            </tr>
            <tr>
                <td style="text-align: center;">
                    <p style="font-weight: 300; margin-bottom: 20px;">Contact Us</p>
